...
# using the image?" if it races the release. Wait for release with two
# signals: the kernel nbd binding (/sys/block/<nbd>/pid exists only while
# connected) is gone, AND a write-lock probe on the image succeeds (a +0
# resize is a no-op that still takes the exact lock the real resize needs).
